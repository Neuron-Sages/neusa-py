...
def scrape_ohlcv(exchange, max_retries, symbol, timeframe, since, limit):
    ...
    while fetch_since < now:
        ...
        if len(all_ohlcv):
            print(len(all_ohlcv), 'candles in total from', exchange.iso8601(
                all_ohlcv[0][0]), 'to', exchange.iso8601(all_ohlcv[-1][0]))
        else:
...
def scrape_candles_to_csv(filename, exchange_id, max_retries, symbol, timeframe, since, limit):
    ...
    print('Saved', len(ohlcv), 'candles from', exchange.iso8601(
        ohlcv[0][0]), 'to', exchange.iso8601(ohlcv[-1][0]), 'to', filename)


# -----------------------------------------------------------------------------
# Binance's BTC/USDT candles start on 2017-08-17
scrape_candles_to_csv('binance.csv', 'binance', 3,
                      'BTC/USDT', '1m', '2022-09-01T00:00:00Z', 500)
